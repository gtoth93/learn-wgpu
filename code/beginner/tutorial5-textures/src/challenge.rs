...
use wgpu::{
    ...
    BindGroupLayoutDescriptor, BindGroupLayoutEntry, BindingResource, BindingType, BlendState, Buffer,
    BufferAddress, BufferUsages, Color, ColorTargetState, ColorWrites, CommandEncoderDescriptor,
    Device, DeviceDescriptor, Face, Features, FragmentState, FrontFace, IndexFormat,
    Instance, InstanceDescriptor, Limits, LoadOp, MemoryHints, MultisampleState,
    ...
    SamplerBindingType, ShaderStages, StoreOp, Surface, SurfaceConfiguration, SurfaceError,
    TextureFormat, TextureSampleType, TextureUsages, TextureViewDescriptor, TextureViewDimension,
    Trace, VertexAttribute, VertexBufferLayout, VertexState,
    VertexStepMode,
};
use winit::{
...
impl State {
    ...
    async fn new(window: Arc<Window>) -> State {
        ...
        let cartoon_texture =
            Texture::from_bytes(&device, &queue, cartoon_bytes, "happy-tree-cartoon.png").unwrap();

        let cartoon_bind_group_desc = BindGroupDescriptor {
            layout: &texture_bind_group_layout,
            entries: &[
                BindGroupEntry {
                    binding: 0,
        ...
        let cartoon_bind_group = device.create_bind_group(&cartoon_bind_group_desc);
