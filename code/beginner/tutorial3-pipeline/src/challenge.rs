...
use wgpu::{
    ...
    SurfaceError, TextureFormat, TextureUsages, TextureViewDescriptor, Trace, VertexState,
};
use winit::{
...
impl State {
    ...
    async fn new(window: Arc<Window>) -> State {
        ...
        let challange_shader = device.create_shader_module(wgpu::include_wgsl!("challenge.wgsl"));

        let challenge_fragment_state = FragmentState {
            module: &challange_shader,
            entry_point: Some("fs_main"),
            targets: &[Some(color_target_state)],
            compilation_options: PipelineCompilationOptions::default(),
        };
        let challenge_render_pipeline_desc = RenderPipelineDescriptor {
            label: Some("Render Pipeline"),
            layout: Some(&pipeline_layout),
            vertex: VertexState {
                module: &challange_shader,
            ...
            fragment: Some(challenge_fragment_state),
            primitive: PrimitiveState {
                topology: PrimitiveTopology::TriangleList,
                strip_index_format: None,
                front_face: FrontFace::Ccw,
        ...
        let challenge_render_pipeline =
            device.create_render_pipeline(&challenge_render_pipeline_desc);
