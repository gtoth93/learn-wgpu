...
use wgpu::{
    ...
    DepthStencilState, Device, DeviceDescriptor, Face, Features, FragmentState, FrontFace,
    IndexFormat, InstanceDescriptor, Limits, LoadOp, MemoryHints, MultisampleState,
    Operations, PipelineCompilationOptions, PipelineLayoutDescriptor, PolygonMode,
    PowerPreference, PrimitiveState, PrimitiveTopology, Queue,
...
impl DepthPass {
    ...
    fn new(device: &Device, config: &SurfaceConfiguration) -> Self {
        let texture =
            Texture::create_depth_texture_non_comparison_sampler(device, config, "depth_texture");
        ...
        Self {
            ...
            vertex_buffer,
            index_buffer,
            num_depth_indices: DEPTH_INDICES.len().try_into().unwrap(),
            render_pipeline,
        }
    }

    fn resize(&mut self, device: &Device, config: &SurfaceConfiguration) {
        self.texture =
            Texture::create_depth_texture_non_comparison_sampler(device, config, "depth_texture");
        let bind_group_desc = BindGroupDescriptor {
            layout: &self.layout,
