...
use wgpu::{
    util::{BufferInitDescriptor, DeviceExt}, Backends, BlendState, Buffer, BufferAddress, BufferUsages, Color,
    ColorTargetState, ColorWrites, CommandEncoderDescriptor, Device, DeviceDescriptor, Face, Features,
    ...
    RenderPipelineDescriptor, RequestAdapterOptions, StoreOp, Surface, SurfaceConfiguration, SurfaceError,
    TextureFormat, TextureUsages, TextureViewDescriptor, Trace, VertexAttribute, VertexBufferLayout,
    VertexState,
    VertexStepMode,
};
use winit::{
...
struct State {
    ...
    index_buffer: Buffer,
    num_indices: u32,
    challenge_vertex_buffer: Buffer,
    challenge_index_buffer: Buffer,
...
impl State {
    ...
    async fn new(window: Arc<Window>) -> State {
        ...
        let num_vertices = 16;
        let angle = std::f32::consts::PI * 2.0 / f32::from(num_vertices);
        let challenge_verts = (0..num_vertices)
            .map(|i| {
                let theta = angle * f32::from(i);
                Vertex {
        ...
        let challenge_indices = (1u16..=num_triangles)
            .flat_map(|i| vec![i + 1, i, 0])
            .collect::<Vec<_>>();
        let num_challenge_indices = challenge_indices.len().try_into().unwrap();
    ...
    fn render(&mut self) -> Result<(), SurfaceError> {
...

        {
            ...
            let (vertex_buffer, index_buffer, num_indices) = if self.use_complex {
                (
